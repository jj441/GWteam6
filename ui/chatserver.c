...
#ifndef __CHATSERVER_C__
#define __CHATSERVER_C__
    ...
    while (1) {
        ...
        case 0:
            ...
            while (1) {
                 ...
                 */
                if ((recvLen = recv(clntSockfd, recvBuffer, BUFSIZE, 0)) ==
                    -1) {
                    perror("recv failed");
                    exit(1);
                }
                ...
                printf("[Oclean 사용자]:%s", recvBuffer);
            }
            close(clntSockfd);
            exit(0);
        ...
        default:
            while (1) {
                ...
                if (send(clntSockfd, sendBuffer, strlen(sendBuffer),
                         MSG_NOSIGNAL) != strlen(sendBuffer)) {
                    /* 상대방의 연결이 끊겨 호출에 실패하면 errno에는 EPIPE가
                     * 저장 */
                    if (errno == EPIPE) {
